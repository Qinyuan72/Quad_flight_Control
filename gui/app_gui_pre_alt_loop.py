...
class RollRateTestApp:
    ...
    def _build_outer_loop_controls(self, parent: ttk.Frame) -> None:
        ...
        ttk.Label(
            frame,
            text="Roll/pitch angle targets generate inner-loop body-rate commands each runtime step.",
            justify="left",
        ...
        for idx, (label, key) in enumerate(rows):
            ...
            ttk.Label(info, textvariable=self.outer_loop_vars[key], font=gui_config.MONO_FONT).grid(
                row=idx,
                column=1,
                sticky="w",
                pady=2,
    ...
    def _render_snapshot(self, snapshot) -> None:
        ...
        self.controller_vars["p_cmd"].set(f"{getattr(outer_loop, 'p_cmd_rad_s', getattr(command, 'p_cmd_rad_s', 0.0)):.4f}")
        ...
        self.controller_vars["q_cmd"].set(f"{getattr(outer_loop, 'q_cmd_rad_s', getattr(command, 'q_cmd_rad_s', 0.0)):.4f}")
